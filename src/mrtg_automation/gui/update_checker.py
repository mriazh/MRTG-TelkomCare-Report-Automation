    ...
    def run(self):
        try:
            req = urllib.request.Request(
                app_info.LATEST_RELEASE_API, headers={"User-Agent": "MRTG-TelkomCare-Updater"}
            )
...
class UpdateManager(QObject):
    ...
    def check_for_updates(self, is_manual=False):
        if self._check_in_progress:
            if is_manual:
                QMessageBox.information(
                    self.parent_widget, "Update Check", "An update check is already running."
                )
            return
        ...
        if is_manual:
            self._progress_dialog = QProgressDialog(
                "Checking for updates...", "", 0, 0, self.parent_widget
            )
    ...
    def _on_check_error(self, error_msg):
        ...
        QMessageBox.warning(
            self.parent_widget, "Update Check Failed", f"Could not check for updates:\n{error_msg}"
        )
