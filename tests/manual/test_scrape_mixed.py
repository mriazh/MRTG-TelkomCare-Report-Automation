...
def main():
    ...
    print(
        f"Parsed SID targets: {len(sid_targets_all)} total, testing {len(sid_targets)}: {sid_targets}"
    )
    print(
        f"Parsed Graph-title targets: {len(graphtitle_targets_all)} total, testing {len(graphtitle_targets)}: {graphtitle_targets}"
    )
